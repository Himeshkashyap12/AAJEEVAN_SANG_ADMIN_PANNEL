...
const ApprovedKycRequest=({activeTab})=>{
    ...
    },
   {
      ...
      width:200,
      align:"start",
      render: (_,record) => <div  className="flex gap-2 items-center cursor-pointer" ><CustomText  value={record?.status} /></div>,
    ...
    },
    {
     ...
     const getAllApprovedKyc=async()=>{
            try{
                ...
                const res=await dispatch(getAllActionAsync({token,data,status:"approved"})).unwrap();
            }catch(error){
        console.log(error);
            }
        }
    ...
    return(
        <>
          <div className="">
            ...
            <CustomCard data={kycRequest?.totalpage} value={"Total Approved Request"} />
            </div>
...
export default ApprovedKycRequest;