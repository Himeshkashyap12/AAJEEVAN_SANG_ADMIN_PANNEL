...
} from "../../feature/pushNotification/pushNotificationSlice";
...
const SendNotification = () => {
  ...
  const [CustomselectedUsers, setCustomSelectedUsers] = useState([]);
            ...
            const deleteItemHandler = (idx, item) => {
                ...
                const selectUserIds = checkedusers
                .filter((c) => c?.checkUsers)
                ?.map((itm) => itm?.id);
            ...
            const columns = [
                ...
                const sendNotificationHandler = async () => {
                    if (!pushNotificationInput?.title || !pushNotificationInput?.message)
                    return toast.error("Please fill all field");
                    if (
                    pushNotificationInput?.target == "custom_users" &&
                    pushNotificationInput?.userIds?.length == 0
                    )
                    return toast.error("Please add Users");
                    try {
                    const data = {
                        ...(pushNotificationInput?.title && {
                        title: pushNotificationInput.title,
                        }),
                        ...(pushNotificationInput?.message && {
                        message: pushNotificationInput.message,
                        }),
                        ...(pushNotificationInput?.target && {
                        target: pushNotificationInput.target,
                        }),
                        ...(pushNotificationInput?.scheduleAt && {
                        scheduleAt: pushNotificationInput.scheduleAt,
                        }),
                        ...(pushNotificationInput?.startDate && {
                        startDate: pushNotificationInput.startDate,
                        }),
                        ...(pushNotificationInput?.endDate && {
                        endDate: pushNotificationInput.endDate,
                        }),
                        ...(pushNotificationInput?.userIds.length > 0 && {
                        userIds: pushNotificationInput.userIds,
                        }),
                    };

                    const res = await dispatch(
                        sendPushNotificationAsync({ token, data })
                    ).unwrap();
                ...
                const getUserData = async () => {
                    ...
                    const res = await dispatch(
                        getAllUserAsync({ token, data, status: "all" })).unwrap();
                    if(res.status && res.code==200){
                       const data=[...CustomselectedUsers,...res.data];
                       const dataSet=new Set(data);
                       console.log(dataSet);
                       
                       setNotifyUsersData([...dataSet])
                    }
                ...
                useEffect(() => {
                    if (users?.data) {
                    const updated = users.data.map((item) => ({
                        ...item,
                        checkUsers: false,
                    }));
                ...
                const selectedUserHandler = (e, item) => {
                    ...
                    data.splice(idx, 1, {
                    ...item,
                    checkUsers: item?.checkUsers ? false : true,
                    });
    ...
    <>
      <div className="flex flex-col  bg-[#fff] shadow-2xl rounded-md p-[24px]">
        ...
        <div className="flex flex-col gap-5 pt-5">
                ...
                return (
                  <>
                    ...
                    >
                      {" "}
                      <CustomText
                        className={`rounded-md px-3 p-1 py-3  ${
                          pushNotificationInput?.target == item?.value
                            ? "border-[1px] !border-[#F81B3E] !text-[#F81B3E]"
                            : "bg-[#F81B3E] !text-[#fff]"
                        }`}
                        value={item?.label}
                      />
          ...
          {pushNotificationInput?.target == "on_date_users" && (
            <div className="leading-0 flex justify-start">
              <RangePicker
                disabledDate={(current) =>
                  current && current > new Date().setHours(0, 0, 0, 0)
                }
                onChange={(value) => {
                  rangePickerHander(value);
                }}
              />
            </div>
          )}
              ...
              <div className="max-h-[300px] overflow-auto w-full flex flex-col gap-3">
                {notifyUsersData?.map((item, idx) => (
                  <div className="flex justify-between w-full">
                    {isLoading ? (
                      <Skeleton paragraph={{ rows: users?.data?.length }} />
                    ) : (
                      <>
          ...
          {pushNotificationInput?.target === "custom_users" && (
            <CustomTable columns={columns} dataSource={CustomselectedUsers} />
          )}
